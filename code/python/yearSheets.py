...
#fileMap = "Year4-Algebra_consumed.json"
...
def is_in(value, allowed):
    if isinstance(allowed, (list, tuple, set)):
        ...
        return value == allowed  # Treat scalar as exact match
...
def create_multiple_pdfs(all_data):
    ...
    for i in range(10):
        ...
        """section1Type = strategy['sN'][i]
        section2Type = strategy['sA'][i]
        section3Type = strategy['m'][i]
        section2Type = strategy['h'][i]"""
        ...
        section1 = select_questions(
            ...
            # questionBlock,
            # {"type": simpleNumeric, "difficulty": strategy["sN"][i]["difficulty"]},# "fnumber": [f for f in allFnumber if f not in questionBlock]},
            {"difficulty": 'easy', "used":0},
            #7
            10
        )
        ...
        section2 = select_questions(
            ...
            # questionBlock,
            # {"type": simpleArithmetic, "difficulty": strategy["sA"][i]["difficulty"], "fnumber": [f for f in allFnumber if f not in questionBlock]},
            {"difficulty": 'medium', "used":0},
            #12
            10
        )
        ...
        section3 = select_questions(
            ...
            # questionBlock,
            # {"difficulty": strategy["m"][i]["difficulty"], "type":[t for t in allType if t not in EXCLUDED_TYPE], "fnumber": [f for f in allFnumber if f not in questionBlock]},
            {"difficulty": 'hard', "used":0},
            #11
            10
        )
        ...
        output_pdf = ''
        if i == 0:
            output_pdf = f"{OUTPUT_PREFIX}_sample.pdf"
        else:
            output_pdf = f"{OUTPUT_PREFIX}{i}.pdf"
...
if __name__ == "__main__":
    ...
    if not all_data:
        ...
        # with open('../../jsons/year456/src/y4Geometry_position_and_direction_consumed.json', 'w', encoding='utf-8') as file:
        #     json.dump(
        #         all_data, 
        #         file, 
