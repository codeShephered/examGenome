...
OUTPUT_PREFIX = "testPaper_phase1"  # Prefix for output PDF files
...
def create_multiple_pdfs(all_data):
    """Create multiple PDFs of 50 questions each, without repeating questions."""
    all_data = balanced_random_selection(all_data)
    total_questions = len(all_data)
    num_pdfs = ceil(total_questions / TOTAL_PER_PDF)

    print(f"📦 Total {total_questions} questions → {num_pdfs} PDF(s) planned")

    for i in range(num_pdfs):
        start_idx = i * TOTAL_PER_PDF
        end_idx = start_idx + TOTAL_PER_PDF
