...
# === Configuration ===
# ============================================================
INPUT_FILE = "../../jsons/year456/src/extracted_y3Geometry_properties_of_shapes.json"
OUTPUT_PREFIX = "Year3 - Geometry properties of shapes"
...
def _build_code_prompt(question_text: str, options: dict) -> str:
    ...
    We ask for self-contained code that saves the figure to a file path
    passed in via a variable called `OUTPUT_PATH`.
    """
    ...
    return textwrap.dedent(f"""
        ...
        a clear, minimal diagram illustrating the question below.

        Rules:
        - Use only matplotlib and the Python standard library (no other imports).
        - White background, clean axes, large readable labels/tick marks.
        - Figure size: 4 × 3 inches at 100 dpi (so 400×300 px).
        - Save the figure to the path stored in the variable OUTPUT_PATH using
        - plt.savefig(OUTPUT_PATH, dpi=100, bbox_inches=tight).
        - Do NOT include plt.show().
        - Do NOT include any text from the question or answer options in the diagram
          — only the geometric/visual content (grid, points, shapes, number line, etc.).
        - Assume OUTPUT_PATH is already defined before your code runs.
...
def _make_placeholder_image(question_text: str) -> str | None:
    """
    Minimal placeholder PNG when all API/code-gen attempts fail.
    Drawn with matplotlib so no extra dependency is needed.
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        cache = _cache_path(question_text + "_placeholder")
        if os.path.exists(cache):
            return cache

        fig, ax = plt.subplots(figsize=(4, 3))
        ax.set_xlim(0, 1); ax.set_ylim(0, 1)
        ax.text(0.5, 0.5, "[ Diagram ]", ha="center", va="center",
                fontsize=14, color="#aaaacc",
                bbox=dict(boxstyle="round,pad=0.4", edgecolor="#aaaacc",
                          facecolor="white"))
        ax.axis("off")
        plt.savefig(cache, dpi=100, bbox_inches="tight")
        plt.close("all")
        return cache
    except Exception:
        return None
...
def write_to_pdf(data_list, output_pdf):
    ...
    if ENABLE_IMAGE_GENERATION:
        ...
        for idx, entry in enumerate(data_list, start=1):
            ...
            if isinstance(numbers, list) and len(numbers) > 0:
                # Grouped question — no image for the group header
                img_paths[idx] = None
            else:
                q_text = entry.get("question", "")
                opts   = get_options(entry)
                path   = generate_question_image(q_text, opts)
                if path is None and _needs_image(q_text, opts):
                    # API unavailable — use placeholder so layout is consistent
                    path = _make_placeholder_image(q_text)
                img_paths[idx] = path
                if path:
    ...
    toc_table = Table(toc_data, colWidths=[250, 100])
