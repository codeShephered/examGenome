...
OUTPUT_PREFIX = "testphase1"  # Prefix for output PDF files
TOTAL_PER_PDF = 50  # Questions per PDF
LOGO_PATH = "examgenie.png"
# === PAGE DIMENSIONS ===
        ...
        if filename.endswith(".json"):
            ...
            with open(file_path, "r", encoding=encoding) as f:
                try:
                    ...
                    fileMap[filename] = decoded_content
...
def write_to_pdf(data_list, output_pdf):
    ...
    Please don’t hesitate to contact us at <u>support@examgenie.co.uk</u><br/>
    if you have any questions or queries.<br/><br/>
    Visit us at <u>www.examgenie.co.uk</u><br/>
...
def create_multiple_pdfs(all_data):
    ...
    for i in range(num_pdfs):
        ...
        """# --- 1️⃣ First 7 questions: simpleNumeric types ---
        numeric_type = random.choice(simpleNumeric)
        section1 = select_questions(all_data, used_questions, "type", [numeric_type], 7, key_type="type")

        # --- 2️⃣ Next 13 questions: simpleArithmetic types ---
        arithmetic_type = random.choice(simpleArithmetic)
        section2 = select_questions(all_data, used_questions, "type", [arithmetic_type], 13, key_type="type")

        # --- 3️⃣ Next 15 questions: medium difficulty ---
        section3 = select_questions(all_data, used_questions, "difficulty", ["medium"], 15, key_type="difficulty")

        # --- 4️⃣ Last 15 questions: hard difficulty ---
        section4 = select_questions(all_data, used_questions, "difficulty", ["hard"], 15, key_type="difficulty")"""
        ...
        section1 = select_questions(
            all_data,
            used_questions,
            #{"type": [numeric_type], "difficulty": ["easy"]},
            #{"type": [numeric_type]},
            
            {"type": simpleNumeric, "difficulty": ["easy","hard","medium"]},
            7
        )

        # --- 2️⃣ Next 13 questions: simpleArithmetic + easy difficulty ---
        #arithmetic_type = random.choice(simpleArithmetic)
        section2 = select_questions(
            all_data,
            used_questions,
            #{"type": [arithmetic_type], "difficulty": ["easy"]},
            {"type": simpleArithmetic, "difficulty": ["easy","medium","hard"]},
            13
        )

        # ✅ Strict requirement check before proceeding
        if len(section1) < 7 or len(section2) < 13:
            print(f"⚠️ Not enough easy numeric ({len(section1)}) or arithmetic ({len(section2)}) questions.")
            print(f"⛔ Skipping PDF {i + 1} generation.")
            break  # stop PDF creation here

        # --- 3️⃣ Next 15 questions: medium difficulty only ---
        """section3 = select_questions(
            all_data,
            used_questions,
            #{"difficulty": ["medium"], "type": [t ]},
            {"difficulty": ["medium"]},
            15
        )"""
        section3 = select_questions(
            ...
            used_questions,
            #{"difficulty": ["medium"], "type": [t ]},
            {"difficulty": ["medium"], "type":[t for t in allType if t not in EXCLUDED_TYPE]},
            15
        )
        ...
        batch = section1 + section2 + section3 + section4
