#!/usr/bin/env python3
"""
generate_mixed_punctuation_pdfs.py

Reads the mixed-punctuation worksheets JSON (top-level "modules" list, each
with "topic_title", "explanation_rules", "examples" and a "questions" list
of {question_number, difficulty, question_text, answer, explanation}) and
writes one PDF per module.

Note: all 10 modules in this particular file share the same topic_title,
so output files are numbered by their position in the list (01, 02, ...).
...
Usage:
    python3 generate_mixed_punctuation_pdfs.py [input.json] [output_dir] [--logo path.png] [--first-page path.pdf] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the modules JSON file.
                     Defaults to "Year3-Mixed-Punctuation-10-Worksheets.json"
                     in the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "mixed_punctuation_pdfs" if omitted.
...
def build_styles():
    ...
    styles.add(ParagraphStyle(
        ...
        fontSize=17, leading=21, spaceAfter=6 * mm,
    ))
    styles.add(ParagraphStyle(
    ...
    styles.add(ParagraphStyle(
        name="RuleText", parent=styles["BodyText"],
        fontSize=11, leading=15, spaceAfter=1.5 * mm,
    ))
    styles.add(ParagraphStyle(
        name="ExampleText", parent=styles["BodyText"],
        fontSize=11, leading=15, spaceAfter=1.5 * mm, textColor=colors.Color(0.15, 0.15, 0.15),
    ))
    styles.add(ParagraphStyle(
...
def build_module_pdf(module, output_path, page_size=A4,
    ...
    # ---- Topic title ----
    story.append(Paragraph(module["topic_title"], styles["TopicTitle"]))

    # ---- Explanation & Rules ----
    story.append(Paragraph("Explanation & Rules:", styles["SectionHeading"]))
    rule_items = [
        ListItem(Paragraph(rule, styles["RuleText"]), leftIndent=4 * mm)
        for rule in module["explanation_rules"]
    ]
    story.append(ListFlowable(rule_items, bulletType="1", start="1"))

    # ---- Examples ----
    story.append(Paragraph("Examples:", styles["SectionHeading"]))
    example_items = [
        ListItem(
            Paragraph(f"<b>{ex['rule']}:</b> {ex['sentence']}", styles["ExampleText"]),
            leftIndent=4 * mm,
        )
    ...
    # ---- Explanation Sheet (new page) ----

    #story.append(PageBreak())
    #story.append(Paragraph(module["topic_title"], styles["TopicTitle"]))
    #story.append(Paragraph("Explanation Sheet", styles["SectionHeading"]))

    #for q in module["questions"]:
    #    story.append(Paragraph(f"<b>{q['question_number']}.</b> {q['explanation']}", styles["SheetText"]))
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Punctuation_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one worksheet PDF per module (worksheet + "
                     "answer sheet), with a logo watermark and "
                     "copyright/page-number footer on every generated page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Mixed-Punctuation-10-Worksheets.json",
                         help="Path to the modules JSON file.")
    parser.add_argument("output_dir", nargs="?", default="mixed_punctuation_pdfs",
    ...
    if not modules:
        print("Error: no modules found in the JSON file.")
        sys.exit(1)

    written = []
    for i, module in enumerate(modules, 1):
        required_keys = {"topic_title", "explanation_rules", "examples", "questions"}
        if not required_keys.issubset(module.keys()):
            print(f"Skipping module {i}: missing one of {required_keys}")
            continue

        # All modules in this file share the same topic_title, so the
        # worksheet number is used to keep filenames distinct.
        slug = slugify(module["topic_title"])
        filename = f"{i:02d}_Worksheet_{i}_{slug}.pdf"
        output_path = os.path.join(output_dir, filename)

        content_path = os.path.join(output_dir, f".content_{i:02d}.pdf")
        build_module_pdf(module, content_path,
        ...
        print(f"Written: {output_path}  ({len(module['questions'])} questions)")
