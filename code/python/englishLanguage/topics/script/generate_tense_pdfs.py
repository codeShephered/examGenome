#!/usr/bin/env python3
"""
generate_tense_pdfs.py

Reads the Tense (Present, Past) worksheets JSON (top-level "worksheets"
list, each with "worksheet_number", "topic", "explanation" -- an object
with "present_tense" and "past_tense" text -- and a "questions" list of
{question_number, verb_given, convert_to, question_text, answer,
explanation}) and writes one PDF per worksheet.

Note on this JSON's shape: unlike the Common Noun / Proper Noun worksheet
files, this one has no "focus_area" field (all 10 worksheets share the
same fixed "topic"), and no separate "examples" list -- the illustrative
example sentences are already embedded inside the explanation text itself
("For example: ..."). This script adapts accordingly: each PDF is titled
by its worksheet number (to tell the 10 apart), with the shared topic
shown as a subtitle, and the explanation section prints both the present
and past tense text (which already carries its own worked example).
...
Usage:
    python3 generate_tense_pdfs.py [input.json] [output_dir] [--logo path.png] [--first-page path.pdf] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the worksheets JSON file.
                     Defaults to "Year3-Tense-Present-Past-10-Worksheets.json"
                     in the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "tense_pdfs" if omitted.
...
def build_styles():
    ...
    styles.add(ParagraphStyle(
        name="ExplanationHeading", parent=styles["Heading2"],
        fontSize=12, leading=15, spaceBefore=2 * mm, spaceAfter=2 * mm,
    ...
    styles.add(ParagraphStyle(
        name="ConvertTag", parent=styles["BodyText"],
...
def build_worksheet_pdf(worksheet, output_path, page_size=A4,
    ...
    """Build a single worksheet PDF (worksheet + answer sheet) for one
    Tense worksheet dict."""
    styles = build_styles()
    ...
    # ---- Title (worksheet number) and topic subtitle ----
    story.append(Paragraph(title, styles["MainTitle"]))
    story.append(Paragraph(worksheet["topic"], styles["TopicSubtitle"]))

    # ---- Explanation (present tense + past tense, each with its own
    # worked example already embedded in the text) ----
    story.append(Paragraph("Present Tense", styles["ExplanationHeading"]))
    story.append(Paragraph(worksheet["explanation"]["present_tense"], styles["ExplanationText"]))
    story.append(Paragraph("Past Tense", styles["ExplanationHeading"]))
    story.append(Paragraph(worksheet["explanation"]["past_tense"], styles["ExplanationText"]))

    # ---- Questions, each with a light-shaded answer box ----
    story.append(Paragraph("Questions:", styles["SectionHeading"]))

    for q in worksheet["questions"]:
        q_number = q["question_number"]
        q_text = q["question_text"]
        convert_to = q.get("convert_to", "")

        block = [Paragraph(f"<b>{q_number}:</b> {q_text}", styles["QuestionText"])]
        if convert_to:
            block.append(Paragraph(f"(convert to {convert_to} tense)", styles["ConvertTag"]))
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Tense_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one Tense (Present, Past) worksheet PDF per "
                     "worksheet (worksheet + answer sheet), with a logo "
                     "watermark and copyright/page-number footer on "
                     "every generated page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Tense-Present-Past-10-Worksheets.json",
                         help="Path to the worksheets JSON file.")
    parser.add_argument("output_dir", nargs="?", default="tense_pdfs",
