#!/usr/bin/env python3
"""
generate_homophones_pdfs.py

Reads the Homophones worksheets JSON (top-level "worksheets" list, each
with "worksheet_number", "topic", "explanation" -- a single string --
"homophone_sets" and a "questions" list of {question_number,
homophone_set, question_text, answer, explanation}) and writes one PDF
per worksheet.

Note on this JSON's shape: like the Tense, Adjective and Comparative
worksheet files, all 10 entries here share the same fixed "topic"
("Homophones"), so each PDF is titled by its worksheet number (to tell
the 10 apart), with the shared topic shown as a subtitle.
...
Usage:
    python3 generate_homophones_pdfs.py [input.json] [output_dir] [--logo path.png] [--first-page path.pdf] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the worksheets JSON file.
                     Defaults to "Year3-Homophones-10-Worksheets.json" in
                     the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "homophones_pdfs" if omitted.
...
def build_worksheet_pdf(worksheet, output_path, page_size=A4,
    ...
    watermark and footer on every page) for one Homophones worksheet
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Homophones_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one Homophones worksheet PDF per worksheet "
                     "(cover page + worksheet + answer sheet), with a "
                     "logo watermark and copyright/page-number footer on "
                     "every content page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Homophones-10-Worksheets.json",
                         help="Path to the worksheets JSON file.")
    parser.add_argument("output_dir", nargs="?", default="homophones_pdfs",
