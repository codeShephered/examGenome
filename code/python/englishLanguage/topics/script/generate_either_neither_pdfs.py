#!/usr/bin/env python3
"""
generate_either_neither_pdfs.py

Reads the "Use of Either or Neither" worksheets JSON (top-level
...
Note on this JSON's shape: like the Tense, Adjective, Comparative,
Homophones and Because/Though/Although worksheet files, all 10 entries
here share the same fixed "topic" ("Use of Either or Neither"), so each
PDF is titled by its worksheet number (to tell the 10 apart), with the
shared topic shown as a subtitle.
...
Usage:
    python3 generate_either_neither_pdfs.py [input.json] [output_dir] [--logo path.png] [--first-page path.pdf] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the worksheets JSON file.
                     Defaults to "Year3-Either-Neither-10-Worksheets.json"
                     in the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "either_neither_pdfs" if omitted.
...
def build_worksheet_pdf(worksheet, output_path, page_size=A4,
    ...
    watermark and footer on every page) for one Either/Neither worksheet
    dict. Does NOT include the cover page -- that is prepended
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Either_neither_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one 'Use of Either or Neither' worksheet "
                     "PDF per worksheet (cover page + worksheet + answer sheet), "
                     "with a logo watermark and copyright/page-number footer on "
                     "every content page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Either-Neither-10-Worksheets.json",
                         help="Path to the worksheets JSON file.")
    parser.add_argument("output_dir", nargs="?", default="either_neither_pdfs",
