#!/usr/bin/env python3
"""
generate_adjective_pdfs.py

Reads the Adjective worksheets JSON (top-level "worksheets" list, each
with "worksheet_number", "topic", "explanation" -- a single string -- and
a "questions" object with two sections: "write_a_sentence" and
"identify_the_adjective", each a list of 10 {question_number,
question_text, answer, explanation} items) and writes one PDF per
worksheet.

Note on this JSON's shape: like the Tense worksheets, all 10 entries here
share the same fixed "topic" ("Adjective"), so each PDF is titled by its
worksheet number (to tell the 10 apart), with the shared topic shown as a
subtitle. The "questions" field is also not a single flat list here -- it
has two clearly distinct sections, each independently numbered 1-10, so
both the worksheet and the Answer Sheet label "Section A" and "Section B"
explicitly to avoid any ambiguity between the two "Question 1"s.

Each PDF contains, in order:
    ...
    Section A: Write a Sentence
        <question_number>: <question_text>   (x10, each with a
                                                two-line-tall answer box)
    Section B: Identify the Adjective
        <question_number>: <question_text>   (x10, each with a
                                                two-line-tall answer box)
    --- new page ---
    Answer Sheet
        Section A: <question_number>. <answer>   (x10)
        Section B: <question_number>. <answer>   (x10)
...
Usage:
    python3 generate_adjective_pdfs.py [input.json] [output_dir] [--logo path.png] [--first-page path.pdf] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the worksheets JSON file.
                     Defaults to "Year3-Adjective-10-Worksheets.json" in
                     the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "adjective_pdfs" if omitted.
...
def build_styles():
    ...
    styles.add(ParagraphStyle(
        ...
        fontSize=18, leading=22, spaceAfter=1 * mm,
    ))
    styles.add(ParagraphStyle(
        name="TopicSubtitle", parent=styles["Heading2"],
        fontSize=11, leading=14, spaceAfter=5 * mm,
        textColor=colors.Color(0.40, 0.40, 0.40), alignment=1,  # centred
    ))
    styles.add(ParagraphStyle(
    ...
    styles.add(ParagraphStyle(
        ...
        fontSize=11, leading=14, spaceBefore=3 * mm, spaceAfter=2 * mm,
        textColor=colors.Color(0.25, 0.25, 0.55),
    ))
    styles.add(ParagraphStyle(
...
def build_worksheet_pdf(worksheet, output_path, page_size=A4,
    ...
    """Build a single worksheet PDF (worksheet + explanation sheet +
    answer sheet) for one Adjective worksheet dict."""
    styles = build_styles()

    title = f"Worksheet {worksheet['worksheet_number']}"
    ...
    section_a = worksheet["questions"]["write_a_sentence"]
    section_b = worksheet["questions"]["identify_the_adjective"]

    story = []

    # ---- Title and topic subtitle ----
    story.append(Paragraph(title, styles["MainTitle"]))
    story.append(Paragraph(worksheet["topic"], styles["TopicSubtitle"]))

    # ---- Explanation ----
    story.append(Paragraph(worksheet["explanation"], styles["ExplanationText"]))

    # ---- Section A: write a sentence ----
    story.append(Paragraph("Section A: Write a Sentence", styles["SectionHeading"]))
    for q in section_a:
        block = [
            Paragraph(f"<b>{q['question_number']}:</b> {q['question_text']}", styles["QuestionText"]),
    ...
    # ---- Section B: identify the adjective ----
    story.append(Paragraph("Section B: Identify the Adjective", styles["SectionHeading"]))
    for q in section_b:
        block = [
            Paragraph(f"<b>{q['question_number']}:</b> {q['question_text']}", styles["QuestionText"]),
            AnswerBox(width=usable_width, height=TWO_LINE_BOX_HEIGHT),
    ...
    story.append(Paragraph("Section A: Write a Sentence", styles["SubHeading"]))
    for q in section_a:
        story.append(Paragraph(f"<b>{q['question_number']}.</b> {q['answer']}", styles["SheetText"]))

    story.append(Paragraph("Section B: Identify the Adjective", styles["SubHeading"]))
    for q in section_b:
        story.append(Paragraph(f"<b>{q['question_number']}.</b> {q['answer']}", styles["SheetText"]))
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "firstPage.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one Adjective worksheet PDF per worksheet "
                     "(cover page + worksheet + answer sheet), with a "
                     "logo watermark and copyright/page-number footer on "
                     "every generated page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Adjective-10-Worksheets.json",
                         help="Path to the worksheets JSON file.")
    parser.add_argument("output_dir", nargs="?", default="adjective_pdfs",
    ...
    parser.add_argument("--logo", default=default_logo,
                         help="Path to a logo image (PNG/JPG) to use as the watermark. "
                              ...
                              "Pass --logo '' to disable and use text instead.")
    parser.add_argument("--first-page", default=default_first_page,
                         help="Path to the cover-page PDF to prepend. Defaults to "
                              "'firstPage.pdf' next to this script. "
    ...
    parser.add_argument("--watermark-opacity", type=float, default=0.12,
                         help="Watermark opacity, 0-1. Default: 0.12.")
    parser.add_argument("--watermark-size", type=float, default=0.8,
                         help="Watermark size as a fraction of page width AND "
                              "height. Default: 0.8 (80%% x 80%%).")
    ...
    for i, worksheet in enumerate(worksheets, 1):
        required_keys = {"worksheet_number", "topic", "explanation", "questions"}
        if not required_keys.issubset(worksheet.keys()):
            print(f"Skipping worksheet {i}: missing one of {required_keys}")
            continue
        if not {"write_a_sentence", "identify_the_adjective"}.issubset(worksheet["questions"].keys()):
            print(f"Skipping worksheet {i}: missing a question section")
            continue

        filename = f"{i:02d}_Worksheet_{worksheet['worksheet_number']}.pdf"
        ...
        total_q = len(worksheet["questions"]["write_a_sentence"]) + len(worksheet["questions"]["identify_the_adjective"])
        print(f"Written: {output_path}  ({total_q} questions)")
