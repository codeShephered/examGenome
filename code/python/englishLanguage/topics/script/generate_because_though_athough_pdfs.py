#!/usr/bin/env python3
"""
generate_because_though_although_pdfs.py

Reads the "Use of Because, Though, Although" worksheets JSON (top-level
"worksheets" list, each with "worksheet_number", "topic", "explanation"
-- a single string -- and a "questions" list of {question_number,
question_text, answer, explanation}) and writes one PDF per worksheet.

Note on this JSON's shape: like the Tense, Adjective, Comparative and
Homophones worksheet files, all 10 entries here share the same fixed
"topic" ("Use of Because, Though, Although"), so each PDF is titled by
its worksheet number (to tell the 10 apart), with the shared topic shown
as a subtitle.

Each PDF contains, in order:
    Cover page      -- firstPage.pdf, prepended unmodified, IF that file
                        is found next to this script (see below). No
                        watermark or footer is added to this page, since
                        it is an external file this script does not
                        control the layout of.
    Worksheet <N>
    Use of Because, Though, Although

    <explanation>

    Questions:
        <question_number>: <question_text>   (repeated for every
                                                question, each with a
                                                light-shaded, two-line-
                                                tall answer box)
    --- new page ---
    Answer Sheet
        <question_number>: <answer>           (repeated for every
                                                question)

Every content page (i.e. everything except the prepended cover) carries:
  - a faint watermark: the examgenome.png logo, drawn at 80% of the
    page's width AND 80% of the page's height (falls back to plain text
    if the logo is unavailable).
  - a footer with the copyright notice "(c) 2026 ExamGenome.com. All
    rights reserved." and a page number. Page numbers count only the
    content pages generated by this script (1, 2, 3, ...) -- they do NOT
    include the prepended cover page, since this script has no way to
    know whether that external file already has its own numbering.

Cover page (firstPage.pdf):
    This script looks for a file named "firstPage.pdf" in the same
    directory as the script itself (pass a different path with
    --first-page if you keep it elsewhere). If found, it is prepended,
    unmodified, as page 1 of every generated worksheet PDF. If it is not
    found, each PDF is produced without a cover page and a warning is
    printed once.

Usage:
    python3 generate_because_though_although_pdfs.py [input.json] [output_dir] [--logo path.png] [--first-page path.pdf] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the worksheets JSON file.
                     Defaults to
                     "Year3-Because-Though-Although-10-Worksheets.json"
                     in the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "because_though_although_pdfs" if omitted.
    ...
    --watermark-size FLOAT
                     ...
                     page height (not aspect-preserving). Defaults to 0.8.
...
def prepare_watermark_image(source_path, cache_path, opacity=0.12, white_threshold=240):
    ...
    Cached to `cache_path` so repeated calls don't reprocess the image."""
...
def draw_watermark(canvas_obj, doc, text="examgenome", logo_path=None,
                    ...
                    text_color=colors.grey, size_fraction=0.8):
    """Draw a diagonal watermark behind the page content: the logo image
    at `size_fraction` of the page's width AND height independently (0.8
    = 80% x 80%, not aspect-preserving), or faint text if no logo."""
...
def build_worksheet_pdf(worksheet, output_path, page_size=A4,
    ...
    """Build a single content PDF (worksheet + answer sheet, with
    watermark and footer on every page) for one Because/Though/Although
    worksheet dict. Does NOT include the cover page -- that is prepended
    separately after this is built."""
    styles = build_styles()
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Because_Though_Although_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one 'Use of Because, Though, Although' worksheet "
                     "PDF per worksheet (cover page + worksheet + answer sheet), "
                     "with a logo watermark and copyright/page-number footer on "
                     "every content page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Because-Though-Although-10-Worksheets.json",
                         help="Path to the worksheets JSON file.")
    parser.add_argument("output_dir", nargs="?", default="because_though_although_pdfs",
                         help="Directory to write the PDFs into.")
    parser.add_argument("--logo", default=default_logo,
                         help="Path to a logo image (PNG/JPG) for the watermark. "
    ...
    parser.add_argument("--watermark-text", default="examgenome",
                         help="Fallback watermark text if no logo is found.")
