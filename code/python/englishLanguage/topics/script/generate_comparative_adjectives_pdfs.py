#!/usr/bin/env python3
"""
generate_comparative_pdfs.py

Reads the Comparative Degree of Adjective worksheets JSON (top-level
"worksheets" list, each with "worksheet_number", "topic", "explanation"
-- a single string -- and a "questions" list of {question_number,
adjective, comparative, question_text, answer, explanation}) and writes
one PDF per worksheet.

Note on this JSON's shape: like the Tense and Adjective worksheet files,
all 10 entries here share the same fixed "topic" ("Comparative Degree of
Adjective"), so each PDF is titled by its worksheet number (to tell the
10 apart), with the shared topic shown as a subtitle. Each question_text
contains an embedded <u>...</u> tag marking the adjective to be rewritten
-- reportlab's Paragraph renders this natively as a real underline, so it
is passed straight through without any extra processing. The "comparative"
field (the correct answer) is deliberately NOT shown anywhere on the
worksheet page, only on the Answer Sheet, so it never gives the answer
away early.
...
Each PDF contains, in order:
    ...
    Questions:
        <question_number>: <question_text>   (repeated for every question,
                                                each with a light-shaded
                                                answer box; the target
                                                adjective appears
                                                underlined inline)
    --- new page ---
    Explanation Sheet
        <question_number>: <explanation>      (repeated for every question)
    --- new page ---
    Answer Sheet
        <question_number>: <answer>           (repeated for every question)

Every page of every generated PDF carries a faint watermark -- the
examgenome.png logo, drawn at 80% of the page's width AND 80% of the
page's height (falls back to plain text if the logo is unavailable).

Usage:
    python3 generate_comparative_pdfs.py [input.json] [output_dir] [--logo path.png] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the worksheets JSON file.
                     Defaults to "Year3-Comparative-Adjective-10-Worksheets.json"
                     in the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "comparative_pdfs" if omitted.
    --logo PATH     Path to a logo image (PNG/JPG) to use as the watermark.
                     Defaults to "examgenome.png" sitting next to this
                     script. Pass --logo '' to disable and use a text
                     watermark instead.
    --first-page PATH
                     Path to the cover-page PDF to prepend. Defaults to
                     "firstPage.pdf" sitting next to this script. Pass
                     --first-page '' to disable prepending entirely.
    --watermark-text "TEXT"
...
Requires: reportlab, Pillow, numpy (pip install reportlab Pillow numpy --break-system-packages)
"""
...
def build_worksheet_pdf(worksheet, output_path, page_size=A4, box_height=14 * mm,
    ...
    answer sheet) for one Comparative Adjective worksheet dict."""
    styles = build_styles()
    ...
    # ---- Questions, each with a light-shaded answer box. The
    # question_text already contains a <u>...</u> tag around the target
    # adjective, which reportlab renders as a real underline. ----
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Comparative_Adjective_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one Comparative Adjective worksheet PDF per "
                     "worksheet (worksheet + explanation sheet + answer "
                     "sheet), with a logo watermark on every page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Comparative-Adjective-10-Worksheets.json",
                         help="Path to the worksheets JSON file.")
    parser.add_argument("output_dir", nargs="?", default="comparative_pdfs",
