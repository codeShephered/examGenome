#!/usr/bin/env python3
"""
generate_common_noun_pdfs.py

Reads the Common Nouns worksheets JSON (top-level "worksheets" list, each
...
Each PDF contains, in order:
    <focus_area>
    (small "Common Nouns" subtitle, since the topic is otherwise unused)
...
Usage:
    python3 generate_common_noun_pdfs.py [input.json] [output_dir] [--logo path.png] [--watermark-text "TEXT"] [--watermark-opacity 0.12] [--watermark-size 0.8]

    input.json      Path to the worksheets JSON file.
                     Defaults to "Year3-Common-Noun-5-Worksheets.json" in
                     the current directory if omitted.
    output_dir      Directory to write the PDFs into (created if missing).
                     Defaults to "common_noun_pdfs" if omitted.
...
def build_styles():
    ...
    styles.add(ParagraphStyle(
        ...
        textColor=colors.Color(0.40, 0.40, 0.40), alignment=1,
    ))
    styles.add(ParagraphStyle(
...
def build_worksheet_pdf(worksheet, output_path, page_size=A4, box_height=14 * mm,
    ...
    answer sheet) for one Common Nouns worksheet dict."""
    styles = build_styles()

    title = f"Worksheet {worksheet['pdf_number']}"
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Common_Noun_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one Common Nouns worksheet PDF per focus area "
                     "(worksheet + explanation sheet + answer sheet), with a "
                     "logo watermark on every page."
    )
    parser.add_argument("input_json", nargs="?", default="Year3-Common-Noun-5-Worksheets.json",
                         help="Path to the worksheets JSON file.")
    parser.add_argument("output_dir", nargs="?", default="common_noun_pdfs",
    ...
    for i, worksheet in enumerate(worksheets, 1):
        required_keys = {"pdf_number", "topic", "explanation", "questions"}
        ...
        filename = f"{i:02d}_Worksheet_{worksheet['pdf_number']}.pdf"
