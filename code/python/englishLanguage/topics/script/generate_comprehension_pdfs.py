#!/usr/bin/env python3
"""
generate_comprehension_pdfs.py

Reads a JSON file of comprehension topics (in the format produced for the
Year 3 English Comprehension set: a top-level "topics" list, each with
"topic_title", "content", and a "questions" list of
{question_number, question_text, skill_tested, answer}) and writes one PDF
worksheet per topic.
...
Each PDF contains, in order:
    ...
    Topic: <topic_title>
    <content>                       (passage, paragraphs preserved)
    Questions:
        <question_number>: <question_text>
        [ shaded answer box ]       (repeated for every question)
    --- new page ---
    Answer sheet:
        <question_number>: <answer> (repeated for every question)

Usage:
    python3 generate_comprehension_pdfs.py [input.json] [output_dir] [--logo path.png] [--first-page path.pdf] [--watermark-text "TEXT"] [--watermark-opacity 0.12]

    input.json      Path to the topics JSON file.
                     Defaults to "year3_english_comprehension.json" in the
                     current directory if omitted.
    output_dir       Directory to write the PDFs into (created if missing).
                     Defaults to "comprehension_pdfs" if omitted.
    --logo PATH      Path to a logo image (PNG/JPG) to use as the watermark.
                     Defaults to "examgenome.png" sitting next to this
                     script, so keep that file alongside the script (it is
                     provided together with it). Pass --logo '' to disable
                     the image and fall back to a plain text watermark.
    --first-page PATH
...
The logo (or text) is automatically faded and, for images, has its white
background stripped to transparent, then drawn diagonally behind the page
content on every generated page (not the prepended cover) of every
generated PDF. Each of those pages also carries a footer with the
...
class AnswerBox(Flowable):
    """A light grey, rounded-corner rectangle that gives the student space
...
# ----------------------------------------------------------------------
def slugify(title):
    """Turn a topic title into a safe filename."""
...
def build_styles():
    ...
    styles.add(ParagraphStyle(
        name="Passage", parent=styles["BodyText"],
        fontSize=11, leading=16, spaceAfter=4 * mm, alignment=4,  # justified
    ))
    styles.add(ParagraphStyle(
        name="SectionHeading", parent=styles["Heading2"],
        fontSize=14, leading=17, spaceBefore=4 * mm, spaceAfter=3 * mm,
    ...
    styles.add(ParagraphStyle(
        name="SkillTag", parent=styles["BodyText"],
        fontSize=8.5, leading=10, textColor=colors.Color(0.45, 0.45, 0.45),
        spaceAfter=1 * mm,
    ))
    styles.add(ParagraphStyle(
        name="AnswerText", parent=styles["BodyText"],
...
def draw_watermark(canvas_obj, doc, text="examgenome", logo_path=None,
    ...
    Otherwise, falls back to a semi-transparent text watermark.
    """
    canvas_obj.saveState()
...
def build_topic_pdf(topic, output_path, page_size=A4,
                     watermark_text="examgenome", logo_path=None):
    """Build a single worksheet PDF for one topic dict."""
    styles = build_styles()
    ...
    # ---- Topic title ----
    story.append(Paragraph(f"{topic['topic_title']}", styles["TopicTitle"]))

    # ---- Passage content (preserve paragraph breaks) ----
    for para in topic["content"].split("\n\n"):
        para = para.strip()
        if para:
            story.append(Paragraph(para, styles["Passage"]))
    ...
    for q in topic["questions"]:
        ...
        block = [
            Paragraph(f"<b>{q_number}.</b> {q_text}", styles["QuestionText"]),
        ]
        if skill:
            block.append(Paragraph(f"({skill})", styles["SkillTag"]))
    ...
    for q in topic["questions"]:
        q_number = q["question_number"]
        answer = q["answer"]
        story.append(Paragraph(f"<b>{q_number}.</b> {answer}", styles["AnswerText"]))
...
def main():
    ...
    default_first_page = os.path.join(script_dir, "Year_3_Comprehension_Worksheet.pdf")

    parser = argparse.ArgumentParser(
        description="Generate one comprehension-worksheet PDF per topic, "
                     "with a watermark and copyright/page-number footer "
                     "on every generated page."
    )
    parser.add_argument("input_json", nargs="?", default="generate_comprehension_pdfs.json",
                         help="Path to the topics JSON file.")
    parser.add_argument("output_dir", nargs="?", default="comprehension_pdfs",
    ...
    if args.logo and os.path.isfile(args.logo):
        ...
        print(f"Using logo watermark: {args.logo}")
    elif args.logo:
    ...
    if not topics:
        print("Error: no topics found in the JSON file.")
        sys.exit(1)
    ...
    for i, topic in enumerate(topics, 1):
        required_keys = {"topic_title", "content", "questions"}
        if not required_keys.issubset(topic.keys()):
            print(f"Skipping topic {i}: missing one of {required_keys}")
            continue

        slug = slugify(topic["topic_title"])
        filename = f"{i:02d}_{slug}.pdf"
        output_path = os.path.join(output_dir, filename)

        content_path = os.path.join(output_dir, f".content_{i:02d}.pdf")
        build_topic_pdf(topic, content_path, watermark_text=args.watermark_text, logo_path=logo_path)

        prepend_cover_page(content_path, first_page_path, output_path)
        os.remove(content_path)

        written.append(output_path)
        print(f"Written: {output_path}  ({len(topic['questions'])} questions)")
